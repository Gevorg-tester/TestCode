describe('Save_Continue', () => {
    it('Save_Continue', () => {
        cy.visit('https://www.todexpo.com/edit-event/287#basic_information');
        ...
        .click()
        cy.contains('Todexpo Live Stream').click();
        cy.get(':nth-child(8) > :nth-child(1) > .ui-select > .ui-select__content > .ui-select__label > .ui-select__display')
        .click();
        cy.contains('Anonymous Orangutan - Company owner').click();
        cy.get(':nth-child(8) > :nth-child(2) > .ui-select > .ui-select__content > .ui-select__label > .ui-select__display')
        .click();
        // cy.contains('Anonymous Skunk - Company owner').click()
        cy.contains('General').click();
        cy.contains('Presentation').click();
        cy.get(':nth-child(2) > .ui-checkbox__checkmark').click();
        cy.get('.ui-textbox__textarea').type('Conserving the lands and waters on which all life depends')
        cy.get('.ti-new-tag-input-wrapper').type('oppropiate {enter} stages {enter}')
    })
})
